...
#include "../src/RabinKarp.h"
using namespace std::chrono;
TEST(AlgorithmTest, BestStringMatching) {
    string text = "ABCDEFGH";
    string pattern = "A";
    auto start = high_resolution_clock::now();
    int result = rabinKarp(pattern,text);
    auto stop = high_resolution_clock::now();
    std::chrono::duration<double> diff = stop - start;
    std::cout << "BestStringMatching ";
    std::cout << "Execution time: " << diff.count() << " s\n";
    EXPECT_EQ(1,result);
}
TEST(AlgorithmTest, AverageStringMatching) {
    string text = "ABCDEFGH";
    string pattern = "D";
    auto start = high_resolution_clock::now();
    int result = rabinKarp(pattern,text);
